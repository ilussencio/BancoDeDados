...
-- Atualizar em 10% os valores das consultas que ocorreram no mes de junho de 2019 para cardiologitas
update consulta
set valor_consulta =  valor_consulta*1.1
where month(dt_consulta) = 06 and year(dt_consulta) = 2019 and codm in (
    select codm from  medico m where m.especialidade = 'Cardiologista'
    );
-- Utilizando juncao
update consulta c, medico m
set c.valor_consulta = c.valor_consulta * 1.1
where c.codm = m.codm and month(c.dt_consulta) = 6 and year(c.dt_consulta) = 2019 and m.especialidade = 'Cardiologista';
